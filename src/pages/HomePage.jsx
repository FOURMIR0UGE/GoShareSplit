  ...
  return (
    <div className="min-h-screen bg-[#06101d] text-slate-100 transition-colors light:bg-[#e8edf3] light:text-slate-900">
      ...
      {modal==='publish' && <Modal title={editingOffer?'Modifier l’annonce':'Ajouter une annonce'} onClose={()=>{setModal(null);setEditingOffer(null);setForm(initialForm);setError('')}} wide><form onSubmit={submitOffer} className="space-y-5">
        ...
        {form.service.trim() && offerServices.length===0 && <div className="flex flex-col gap-3 rounded-2xl border border-dashed border-emerald-400/50 p-4 sm:flex-row sm:items-center sm:justify-between"><span className="text-sm font-semibold text-slate-300 light:text-slate-700">Service introuvable ?</span><button type="button" onClick={()=>openSuggestion('publish', form.service)} className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-400 px-4 py-2.5 text-sm font-black text-slate-950 hover:bg-emerald-300"><Wrench size={16}/>Suggérer un service</button></div>}
